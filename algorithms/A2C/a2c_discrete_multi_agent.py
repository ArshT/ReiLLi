...
class A2C_multi_agent:
  def __init__(self,env_name,num_agents,max_rounds,max_timesteps_per_ep,update_timestep,solved_reward,fc1_dims,fc2_dims,actor_alpha,critic_alpha,gamma):
    ...
    self.a2c = Agent(input_dims=input_dims,n_actions=action_dims,actor_alpha=actor_alpha,critic_alpha=critic_alpha,gamma=gamma,
    ...
    for round in range(self.max_rounds):
      ...
      for agent_id in range(self.num_agents):
        ...
        agent = Multi_Agent(name = str(agent_id), memory = self.memory,pipe_end = p_end,env_name = self.env_name,max_timesteps_per_ep = self.max_timesteps_per_ep,
                      update_timestep=self.update_timestep,agent=self.a2c,seed=agent_id*10,device=self.device)
      ...
      while True:
        ...
        if False not in agents_completed:
          self.a2c.update_multi(self.memory,self.num_agents)
          agents_completed = [False] * self.num_agents
